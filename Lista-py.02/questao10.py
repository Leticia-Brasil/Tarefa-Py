...
estudantes = []  # lista de dicionários: [{"nome": "Ana", "nota": 8.5}]
NOTA_APROVACAO = 7.0
...
def calcular_media():
    """Retorna a média da turma (ou None se não houver estudantes)."""
    if not estudantes:
        return None
    soma = 0  # acumulador
...
def maior_nota():
    """Retorna o dicionário do estudante com a maior nota."""
    if not estudantes:
        return None
    melhor = estudantes[0]
    for estudante in estudantes:
...
def listar_aprovados():
    """Retorna a lista de estudantes com nota >= 7.0."""
    aprovados = []
    for estudante in estudantes:
